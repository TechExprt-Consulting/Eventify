...
const LandingpageMoviecardCarousel = () => {
   ...
   console.log("jdjjjd",moviesData)
    ...
    const cardmovieBanner =(movieid)=>{
  

        localStorage.setItem('homeMovieId', movieid);
            ...
            >
                {
                    moviedata.map((m,index)=>(
                        <SwiperSlide key={index} onClick={()=>(cardmovieBanner(m.id))} style={{height:"64vh",width:"15.5vw",borderRadius:"1vw"}}>
                        <div>
...
export default LandingpageMoviecardCarousel
